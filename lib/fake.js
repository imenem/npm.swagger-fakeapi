...
function fakeString(name, contextString) {
  ...
  else if (/(?:user[\-_]?names?)$/i.test(name)) {
    return 'slug'
  }
  ...
  else if (/(?:path|path[\-_]?name)s?$/i.test(name)) {
    return 'path'
  }
  else if (/(?:img|pictures?|images?)[\-_]?(ur[il]s?)?$/i.test(name)) {
    return 'imageUrl'
  }
  else if (/(?:ur[il]s?)$/i.test(name)) {
    return /\b(image|img|media)s?\b/i.test(contextString) ? 'imageUrl' : 'url'
  }
