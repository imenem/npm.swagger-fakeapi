...
function fakeTimeZone (rnd) {
  const timeZones = momentTz.tz.names()
  return timeZones[fakeInteger(rnd, 0, timeZones.length)]
}
...
function fakeString (context) {
  ...
  tokens.forEach((token, place) => {
    ...
    else if (/(?:time[_-]?zone)$/i.test(token)) {
      grade('timeZone', 15)
    }
  })
...
function fakeHeader (headerName) {
  if (headerName.toLowerCase() === 'content-disposition') {
    return `attachment; filename="${faker.system.commonFileName('txt')}"`
  }
  else if (headerName.toLowerCase() === 'content-type') {
...
// taxonomy
exports.id = fakeId
exports.monotonicId = fakeIdMonotonic
...
exports.timeZone = fakeTimeZone

//
exports.guessStringFormat = fakeString
exports.header = fakeHeader
