...
function objectMaker(rnd, schema, stack = []) {
  const ret = {}
...
function fieldHandler(rnd, key, field, stack = []) {
  ...
  if (field.type === 'array') {
    ...
    val = range(numItems).map(() => {
      ...
      if (subtype === 'object') {
        return objectMaker(rnd, field.items, stack.concat([key]))
      }
  ...
  // == objects ==
    ...
    val = objectMaker(rnd, field, stack.concat([key]))
    if (field.additionalProperties) {
      range(1 + Math.floor(rnd.random() * 3)).forEach(() => {
        const tempId = fake.slug(rnd, 2, 2)
  ...
  //   number
    ...
    if (useDefault(field, 'imum')) {
      ...
      val = (field.type === 'number')
          ? fake.number(rnd, min, max)
          : fake.integer(rnd, min, max)
    }
  }
  // == booleans ==
  ...
  //   boolean
    ...
    if (useDefault(field, 'Length')) {
      ...
      let handlerId = fake.guessStringFormat(key, contextString(stack, key))
...
function requestHandler(req, res, next) {
  ...
  if ((pathLast === '/' && reqLast !== '/') ||
    ...
    if (example) {
      ...
      const body = fieldHandler(bodyRand, req.route.path, apiRes.schema)
      res.status(200)
      res.send(body)
    }
  }
...
module.exports = function (options) {
  ...
  glob.sync(options.contracts, { follow: true }).forEach(contract => {
    servers.push(new Promise((resolve, reject) => {
      ...
      swaggerParser.validate(contract, (swagErr, api) => {
        if (swagErr) {
          ...
          for (const [pathKey, apiMethod] of entries(api.paths)) {
            for (const [method] of entries(apiMethod)) {
              if (options.verbose) {
                const name = path.relative(baseDir, contract).replace(/([\\/*.]*contract)?[\\/*.]*(json|ya?ml)$/, '')
                console.log(`[swagger-fakeapi] Adding [${name}] ${method.toUpperCase()} ${api.basePath}${pathKey}`)
              }
              if (method in server) {
                // contracts can define methods such as PARAMETERS
                // which Koa does/may not support
                // eslint-disable-next-line consistent-return
                server[method](api.basePath + pathKey, requestHandler)
              }
            }
          }
