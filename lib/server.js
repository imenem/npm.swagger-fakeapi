...
function fieldHandler(rnd, key, field, stack = []) {
  ...
  //   boolean
    ...
    if (useDefault(field, 'Length')) {
      ...
      const handlerId = fake.guessStringFormat(key, contextString(stack, key))
      val = fake[handlerId](rnd, field.minLength || 0, field.maxLength)
    }
  }
      ...
      swaggerParser.validate(contract, (swagErr, api) => {
        ...
        for (const [pathKey, apiMethod] of entries(api.paths)) {
          for (const [method] of entries(apiMethod)) {
            ...
            server[method](api.basePath + pathKey, (req, res, next) => {
              const pathLast = pathKey.slice(-1)
