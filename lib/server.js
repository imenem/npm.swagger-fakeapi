    ...
    swaggerParser.validate(contractFileName)
      .then(api => {
        for (const pathKey in api.paths) {
          if ({}.hasOwnProperty.call(api.paths, pathKey)) {
            const apiMethod = api.paths[pathKey]
            for (const method in apiMethod) {
              if ({}.hasOwnProperty.call(apiMethod, method)) {
                server[method](api.basePath + pathKey, (req, res) => {
                  ...
                  for (const header in apiRes.headers) {
                    if ({}.hasOwnProperty.call(apiRes.headers, header)) {
