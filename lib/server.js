...
function fieldHandler(rnd, key, field, stack = [], ctx = {}) {
  let val
  if (!field) {
    throw new Error(`${key} was not found in ${stack.join('/')}`)
  }
...
function requestHandler(req, res, next) {
  ...
  if ((pathLast === '/' && reqLast !== '/') ||
    ...
    if (example) {
      ...
      let body = fieldHandler(bodyRand, pathContext, apiRes.schema, [], req)
      if (typeof body === 'number') { body = String(body) }
      res.status(200)
      res.send(body)
    }
  }
        ...
        if (swagErr) {
          ...
          for (const [pathKey, apiMethod] of entries(api.paths)) {
            for (const [method] of entries(apiMethod)) {
              const endpoint = path.join(basePath, pathKey)
              if (options.verbose) {
